                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zREFSHARE ought to be enough for anybody." - Bill Gates, 2017
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_20_ZEROCOIN_MAINTENANCE_MODE)) {
        ...
        ui->pushButtonMintzVIT->setToolTip(tr("zREFSHARE is currently disabled due to maintenance."));

        ui->pushButtonSpendzVIT->setEnabled(false);
        ui->pushButtonSpendzVIT->setToolTip(tr("zREFSHARE is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_pushButtonMintzVIT_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_20_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zREFSHARE is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zREFSHARE...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zREFSHARE in ") +
...
void PrivacyDialog::on_pushButtonSpendzVIT_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_20_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zREFSHARE is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zREFSHARE
        sendzVIT();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zREFSHARE
    sendzVIT();
}
...
void PrivacyDialog::sendzVIT()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Vitae Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    // Warn for additional fees if amount is not an integer and change as zREFSHARE is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " REFSHARE </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zREFSHARE</b>";
    ...
    // use mints from zREFSHARE selector if applicable
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zREFSHARE was spent successfully update the addressbook with the label
    ...
    // Clear zREFSHARE selector in case it was used
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zREFSHARE Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " REFSHARE, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zREFSHARE Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zREFSHARE </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zREFSHARE "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zREFSHARE "));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zREFSHARE </b> "));
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zREFSHARE </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
